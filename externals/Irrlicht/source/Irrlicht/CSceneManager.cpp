...
ICameraSceneNode* CSceneManager::addCameraSceneNodeRH(ISceneNode* parent,
	const core::vector3df& position, const core::vector3df& lookat, s32 id,
	bool makeActive)
{
	if (!parent)
	...
	ICameraSceneNode* node = new CCameraSceneNodeRH(parent, this, id, position, lookat);
...
ICameraSceneNode* CSceneManager::addCameraSceneNodeMayaRH(ISceneNode* parent,
	f32 rotateSpeed, f32 zoomSpeed, f32 translationSpeed, s32 id, f32 distance,
	bool makeActive)
{
	ICameraSceneNode* node = addCameraSceneNodeRH(parent, core::vector3df(),
		core::vector3df(0,0,100), id, makeActive);
	if (node)
	{
		ISceneNodeAnimator* anm = new CSceneNodeAnimatorCameraMayaRH(CursorControl,
			rotateSpeed, zoomSpeed, translationSpeed, distance);
...
ICameraSceneNode* CSceneManager::addCameraSceneNodeFPSRH(ISceneNode* parent,
	f32 rotateSpeed, f32 moveSpeed, s32 id, SKeyMap* keyMapArray,
	s32 keyMapSize, bool noVerticalMovement, f32 jumpSpeed,
	bool invertMouseY, bool makeActive)
{
	...
	if (node)
	{
		ISceneNodeAnimator* anm = new CSceneNodeAnimatorCameraFPSRH(CursorControl,
			rotateSpeed, moveSpeed, jumpSpeed,
			keyMapArray, keyMapSize, noVerticalMovement, invertMouseY);

		//ISceneNodeAnimatorCameraFPS* fpsAnm = static_cast<ISceneNodeAnimatorCameraFPS*>( anm );
		// Bind the node's rotation to its target. This is consistent with 1.4.2 and below.
		node->bindTargetAndRotation(true);
		node->addAnimator(anm);
		anm->drop();
