...
void EffectHandler::setScreenRenderTargetResolution(const irr::core::dimension2du& resolution)
{
	...
	ScreenQuad.getMaterial().setTexture(0,0);
	ScreenQuad.getMaterial().setTexture(1,0);
	ScreenQuad.getMaterial().setTexture(2, 0);
...
void EffectHandler::update(irr::video::ITexture* outputTarget)
{
	...
	if(PostProcessingRoutinesSize)
	{
		...
		for(u32 i = 0;i < PostProcessingRoutinesSize;++i)
		{
			...
			auto curTarget = i >= PostProcessingRoutinesSize - 1 ? outputTarget : ScreenQuad.rt[int(Alter)];
			driver->setRenderTarget(curTarget, true, true, ClearColour);

			if ( false )
			{
				IrrEngine::WriteTextureToDisk(DepthRTT, "depth.bmp");
				if ( curTarget )
				{
					IrrEngine::WriteTextureToDisk(curTarget, "ssao.bmp");
				}
			}
...
irr::video::ITexture* EffectHandler::generateRandomVectorTexture(const irr::core::dimension2du& dimensions, const irr::core::stringc& name)
{
...
EffectHandler::SPostProcessingPair EffectHandler::obtainScreenQuadMaterialFromFile(const irr::core::stringc& filename, irr::video::E_MATERIAL_TYPE baseMaterial)
{
	CShaderPreprocessor sPP(driver);
...
void EffectHandler::setPostProcessingEffectConstant(const irr::s32 materialType, const irr::core::stringc& name, const f32* data, const irr::u32 count)
{
...
s32 EffectHandler::addPostProcessingEffectFromFile(const irr::core::stringc& filename, IPostProcessingRenderCallback* callback)
{
